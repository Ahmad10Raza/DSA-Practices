...
public class ArrayDS {
    ...
    // Traverse the given array from start to end and copy each element to the
    // temporary array from end to start.
    ...
    // Initialize two pointers, first and last, pointing to the first and last
    ...
    // Recur for the remaining elements by incrementing the start index and
    ...
    // Initialize two variables, max and min, to store the maximum and minimum
    // elements of the array.
    // Traverse the array from start to end.
    // If the current element is greater than the max element, update the max
    // element.
    ...
    // Initialize two pointers, left and right, pointing to the start and end of the
    // array respectively.
    // Traverse the array from start to end.
    // If the element at the left pointer is negative, increment the left pointer.
    // If the element at the left pointer is positive and the element at the right
    // pointer is negative, swap the elements at the left and right pointers and
    // increment the left pointer and decrement the right pointer.
    // If the element at the left pointer is positive and the element at the right
    // pointer is positive, decrement the right pointer.
    ...
    // To find intersection of two sorted arrays using two-pointers, follow the
    // below approach :
    ...
    // Take two pointers i and j which point to first and last element of array
    // respectively.
    ...
    // Output: 7
    // Explanation: The contiguous subarray [4, -1, -2, 1, 5] has the largest sum =
    // 7.

    // Approach: Kadane’s Algorithm
    // T.C = O(n), S.C = O(1)

    // Initialize two variables, maxSum and currentSum, to store the maximum sum of
    // the subarray and the sum of the current subarray.
    // Traverse the array from start to end.
    // Update the currentSum by adding the current element to it.
    // If the currentSum is less than 0, update the currentSum to 0.
    // If the currentSum is greater than the maxSum, update the maxSum to the
    // currentSum.
    // Print the maxSum.
    ...
    // Create an array dp[] of the same size as the given array to store the maximum
    // sum of the subarray ending at the current index.
    // Initialize the first element of the dp[] array to the first element of the
    // given array.
    ...
    // Initialize three variables, maxReach, steps, and jumps, to store the maximum
    // reachable index,
    // the number of steps that can be taken from the current index, and the number
    // of jumps made so far.
    // Traverse the array from start to end.
    // Update the maxReach to the maximum of maxReach and the sum of the current
    // index and the
    ...
    // Create an array dp[] of the same size as the given array to store the minimum
    // number of
    ...
    // If the current element is greater than or equal to the difference between the
    // current
    // index and the start index, update the current element of the dp[] array to
    // the minimum of the current element and the sum of the element at the start
    // index and 1.
    ...
    // Approach 2: We will use the concept that all elements here are between 1 and
    // n-1.
    ...
    // if the value of a[p] is -1 then increment the pointer by 1 and skip the
    // iteration
    // Else,go to the position of the element to which the current pointer is
    // pointing i.e. at index a[p].
    // Now if the value at index a[p] i.e. a[a[p]] is -1 then break the loop as the
    // element a[p] is the duplicate one.
    // Otherwise store the value of a[a[p]] in a[p] i.e. a[p]=a[a[p]] and put -1 in
    // a[a[p]] i.e. a[a[p]]=-1.
    ...
    // Initialize two variables, i and j, to store the starting indices of the two
    // arrays.
    ...
    // If the current element is greater than the element at the index gap, swap the
    // elements.
    // Traverse the second array from the minimum of the current index and the gap
    // to the end.
    // If the current element is greater than the element at the index gap, swap the
    // elements.
    ...
    // swap the elements and sort the second array as the elements of first array
    // should be less than or equal to elements in second array. Repeating this
    // process while this condition holds true will give us two sorted arrays

    // Follow the below steps to solve the problem:

    // Initialize i with 0
    // Iterate while loop until the last element of array 1 is greater than the
    ...
    // increment the index and update the interval at the index to the current
    // interval.
    // If the current interval overlaps with the previous interval, update the
    // ending time of the
    // interval at the index to the maximum of the ending time of the interval at
    // the index and the ending time of the current interval.
    ...
    // coin of values v1, v2, ....vn. You play against an opponent in an alternating
    // way.
    ...
    // value as 15(10 + 5). It is guarantee that we cannot get more than 15 by any
    // possible moves.

    // Approach: Dynamic Programming

    // Create a 2D array dp[][] of size n x n to store the maximum possible amount
    // of money that can be won by the first player.
    // Initialize the diagonal elements of the dp[][] array to the values of the
    // coins.
    // Traverse the array from the second diagonal to the first diagonal.
    // For each element dp[i][j], update it to the maximum of the sum of the value
    // of the coin at
    // the ith index and the minimum of the value of the coin at the (i+2)th index
    // and the value
    // of the coin at the jth index and the fsum of the value of the coin at the
    // (i+1)th index and
    // the minimum of the value of the coin at the (i+1)th index and the value of
    // the coin at the (j-1)th index.
    ...
    // Create a recursive function, solve(arr, i, j), to find the maximum possible
    // amount of money that can be won by the first player.
    // If the starting index is greater than or equal to the ending index, retur n
    // 0.
    // If the value of the coin at the starting index is greater than the value of
    // the coin at the ending index,
    // return the maximum of the sum of the value of the coin at the starting index
    // and the result of the recursive function
    // called with the starting index incremented by 1 and the ending index, and the
    // sum of the value of the coin at the
    // starting index and the result of the recursive function called with the
    // starting index and the ending index decremented by 1.
    // If the value of the coin at the starting index is less than or equal to the
    // value of the coin at the ending index,
    // return the maximum of the sum of the value of the coin at the ending index
    // and the result of the recursive function
    // called with the starting index incremented by 1 and the ending index
    // decremented by 1, and the sum of the value of the
    // coin at the starting index and the result of the recursive function called
    // with the starting index and the ending index decremented by 1.
    ...
    // greater
    // permutation, then print the lexicographically smallest permutation of the
    // given array.
    ...
    // Initialize two variables, i and j, to store the indices of the elements to be
    // swapped.
    // Traverse the array from the second last element to the first element.
    // Find point of change i.e. the element at the index i such that the element at
    // the
    // index i is less than the element at the index i+1.(First deacreasing
    // sequence)
    // Find number to be swapped with i.e. the element at the index j such that the
    // element
    // at the index j is greater than the element at the index i.(Just first greater
    // element than i)
    ...
    // Where two elements arr[i] and arr[j] form an inversion if a[i] > a[j] and i <
    // j.
    ...
    // Create a recursive function, mergeSortAndCount(arr, l, r), to find the
    ...
    // Calculate the middle index, m, as the sum of the left and right indices
    // divided by 2.
    // Recursively call the mergeSortAndCount function with the left index and the
    // middle index and add the result to the count.
    // Recursively call the mergeSortAndCount function with the middle index + 1 and
    // the right index and add the result to the count.
    // Merge the two sorted halves of the array and add the number of inversions to
    // the count.
    // Return the count.
    ...
    // Initialize a HashMap, map, to store the frequency of the elements in the
    // array.
    // Traverse the array from start to end.
    // If the element K minus the current element is present in the map and the
    // frequency of the
    ...
    public int getPairsCount(int[] arr, int n, int k) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            // If element is present in map, increment the count by the frequency of the
            // element
    ...
    // Find the index of the first element having value same or just greater than (K
    // – arr[i]) using lower bound.
    // Find the index of the first element having value just greater than (K –
    // arr[i]) using upper bound.
    // The gap between these two indices is the number of elements with value same
    // as (K – arr[i]).
    ...
    // search of another common integer. Otherwise, the smaller integer among the
    // three must not be common.

    // The reason for this is that at least one of the other integers is a larger
    // integer,
    // and as we move forward in the array, we only encounter larger integers.
    // In this case, we want to proceed with only the array that contains the
    // smaller integer.

    // Create and initialize three variables i, j, and k with 0, it will point to
    ...
    // the intersection in a temporary array, then find the intersection of the
    // third array and temporary array.
    ...
    // Q_19: Rearrange the array in alternating positive and negative items with
    // O(1) extra space
    // Given an array having positive and negative numbers, our task is to arrange
    // them in an alternate
    // fashion such that every positive number is followed by a negative number and
    // vice-versa maintaining
    // the order of appearance. The number of positive and negative numbers need not
    // to be equal.
    // If there are more positive numbers then they have to appear at the end of the
    // array , same condition for negative numbers also .
    ...
    // Initialize two variables, i and j((arr.length-1)), to store the indices of
    // the elements to be swapped.
    // Traverse the array from the start to the end.
    // If i is negative, increment i by 1 and swap to j or if j is positive
    // increment j by one and swap otherwise increment i and j one by one.
    ...
    // If the index is found, swap the elements at the current index and the index
    // found.
    // increment k by 2.
    // Print the array.
    ...
    // Traverse the array from start to end.
    // If the sum is equal to 0 or the sum is already present in the set, return
    // true.
    ...
    // simply look for arr[i] – 1 in the hash, if not found, then this is the first
    // element of a subsequence.
    // If this element is the first element, then count the number of elements in
    // the
    // consecutive starting with this element. Iterate from arr[i] + 1 till the last
    // element that can be found.
    // If the count is more than the previous longest subsequence found, then update
    // this.
    ...
    // Initialize a variable, max_count, to store the maximum count of the longest
    // subsequence.
    // Traverse the array from the second element to the end.
    // If the current element is equal to the previous element plus 1, increment the
    // count.
    // Otherwise, update the max_count with the maximum of the count and max_count
    ...
    // Q_23: Given an array of size n and a number k, fin all elements that appear
    // more than " n/k " times.
    // Given an array of size n and an integer k, find all elements in the array
    // that appear more than n/k times.

    // Examples:
    // Input: arr[] = {3, 1, 2, 2, 1, 2, 3, 3}, k = 4
    // Output: {2, 3}
    // Explanation: Here n/k is 8/4 = 2, therefore 2 appears 3 times in the array
    // that is
    // greater than 2 and 3 appears 3 times in the array that is greater than 2
    // Input: arr[] = {9, 8, 7, 9, 2, 9, 7}, k = 3
    // Output: {9}
    // Explanation: Here n/k is 7/3 = 2, therefore 9 appears 3 times in the array
    ...
    // day,
    // the second transaction can only start after the first one is complete
    // (Buy->sell->Buy->sell).
    // Given stock prices throughout the day, find out the maximum profit that a
    ...
    // Create two arrays, profit1 and profit2, to store the maximum profit for the
    // first and second transactions.
    // Initialize the profit1 and profit2 arrays with 0.
    // Initialize two variables, min_price1 and min_price2, to store the minimum
    // price for the first and second transactions.
    // Traverse the array from the start to the end.
    // Update the min_price1 with the minimum of the current element and min_price1.
    // Update the profit1 with the maximum of the current element minus min_price1
    // and profit1.
    // Update the min_price2 with the minimum of the current element minus profit1
    // and min_price2.
    // Update the profit2 with the maximum of the current element minus min_price2
    // and profit2.
    ...
    // first and second sell as 0. This is to ensure to get profit from
    // transactions.
    // Iterate through the array and return the second sell as it will store maximum
    // profit.
    ...
    // and then iterate on the second array and find if the element exists in the
    // HashSet,
    // if the HashSet doesn’t contain any particular value then the second array is
    // not the subset of the first array.
    ...
    // If we encounter a particular value that is present in arr2[] but not in the
    // HashSet,
    ...
    // and fix the first element of the triplet. Now use the Two Pointers algorithm
    // to
    // find if there is a pair whose sum is equal to x – array[i]. Two pointers
    // algorithm
    // take linear time so it is better than a nested loop.

    // Algorithm:
    // Sort the given array.
    // Iterate over the array and fix the first element of the possible triplet,
    // arr[i].
    // Then fix two pointers, one at i + 1 and the other at n – 1. And look at the
    // sum,
    // If the sum is smaller than the required sum, increment the first pointer.
    // If the sum is bigger, decrements the end pointer to reduce the sum.
    // If the sum of elements at two-pointer is equal to the required sum then print
    // the triplet and return.
    ...
    // Approach: Two-Pointer Technique
    // T.C = O(n), S.C = O(1)
    // At every index, The amount of rainwater stored is the difference between the
    // current
    // index height and a minimum of left max height and right max-height.

    // Here we can use the two-pointer approach to find the minimum among the
    // left-max and
    // right-max of the probable outermost boundary for any index and iterate
    // likewise.

    // For example:

    // Say we have indices i, j and a boundary of (left, right). where i is the left
    // pointer and j is the right pointer.
    // If the minimum is arr[left], we can say that i is bounded in one side by left
    // and no matter whatever the values
    // are in between (i, right), the rightmost boundary of i will at least have
    // height arr[right] which is the
    // probable outermost boundary for i.
    // So the water height of water column at index i is arr[left] – arr[i] and we
    // can increment i then.
    // Similar things happen for j also.

    // Follow the steps mentioned below to implement the idea:

    // Take two pointers l and r. Initialize l to the starting index 0 and r to the
    // last index N-1.
    // Since l is the first element, left_max would be 0, and right_max for r would
    // be 0.
    ...
    // For the right max of l, We can say that the right max would always be >=
    // current r_max here
    ...
    // For the left max of l, We can say that the left max would always be >=
    // current l_max here
    ...
    // The above approach can be optimized by pre-computing the left and right max
    // for each element.
    // Follow the steps below to solve the problem:
    // Follow the steps mentioned below to implement the approach:
    ...
    // Traverse the array from start to end.
    // The amount of water that will be stored in this column is min(left[i],
    // right[i]) – array[i]
    ...
    // We keep incrementing r until the sum of the window is greater than the given
    // value.
    // Once the sum is greater, we increment l until the sum is less than the given
    // value.
    ...
    // Output: 1
    // Explanation:
    // To bring elements 2, 1, 3 together, swap element ‘5’ with ‘3’ such that final
    // array will be arr[] = {2, 1, 3, 6, 5}
    ...
    // Find the count of all elements which are less than or equal to ‘k’. Let’s say
    // the count is ‘cnt’
    // Using the two-pointer technique for a window of length ‘cnt’, each time keep
    // track of how many
    // elements in this range are greater than ‘k’. Let’s say the total count is
    // ‘bad’.
    // Repeat step 2, for every window of length ‘cnt’ and take a minimum of count
    ...
    // palindromic,
    // similar to single-character string). In that case, the size of array will be
    // reduced to 1.
    // But in this problem, we are asked to do it in the minimum number of
    // operations.
    ...
    // Let f(i, j) be minimum merging operations to make subarray arr[i..j] a
    // palindrome.
    // If i == j answer is 0. We start i from 0 and j from n-1.

    // If arr[i] == arr[j], then there is no need to do any merging operations at
    // index i or index j.
    // Our answer in this case will be f(i+1, j-1).
    // Else, we need to do merging operations. Following cases arise.
    // If arr[i] > arr[j], then we should do merging operation at index j. We merge
    // index j-1 and j,
    // and update arr[j-1] = arr[j-1] + arr[j]. Our answer in this case will be 1 +
    // f(i, j-1).
    // For the case when arr[i] < arr[j], update arr[i+1] = arr[i+1] + arr[i]. Our
    // answer in this case will be 1 + f(i+1, j).
    // Our answer will be f(0, n-1), where n is the size of array arr[].
    // Therefore this problem can be solved iteratively using two pointers (first
    // pointer
    // pointing to start of the array and second pointer pointing to the last
    // element
    // of the array) method and keeping count of total merging operations done till
    // now.
    ...
    // the
    // median of the array obtained after merging the above 2 arrays(i.e. array of
    // length 2n).
    ...
    // If count is equal to n1, we have reached the median. Take the average of the
    // elements at the last index and last-1 index in the merged array.
    // Below is the implementation of the above approach:
    ...
    // Approach 2: Binary Search
    // We can find the kth element by using binary search on whole range of
    ...
    // sorted arrays,
    // where N is the number of elements in the first array, and M is the number of
    ...
    // Explanation: The merged array is: ar3[] = {-12, -10, -6, -5 , -3, 3, 4, 6,
    // 10, 12, 15}.
    ...
    // The idea is to use the binary search approach to find the partition of the
    // arrays.
    // The partition should be such that the elements on the left side should be
    // less than the elements on the right side.
    // The partition should be such that the number of elements on the left side
    // should be equal to the number of elements on the right side.
    // The implementation of the above approach is as follows:

    // The given two arrays are sorted, so we can utilize the ability of Binary
    // Search to divide
    // the array and find the median.

    // Median means the point at which the whole array is divided into two parts.
    // Hence since the two arrays are not merged so to get the median we require
    // merging which is costly.

    // Hence instead of merging, we will use a modified binary search algorithm to
    // efficiently find the median.

    // The idea is to divide the two arrays into two halves such that the number of
    // elements in the left half is equal to the number of elements in the right
    // half.
